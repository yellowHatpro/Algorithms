...
Auxiliary Space : O(1)
Boundary Cases 	: Takes max time to sort if elements are sorted in Reverse order. 
				  Takes min time (O(n)) when elements are already sorted.\
Algorithm Paradigm : Incremental Approach
Sorting in place : Yes
Stable : Yes
Online : Yes
Uses : When number of elements are smaller in the array.
	   When the array is almost sorted, only few elements are misplaced in completely big array.				  
